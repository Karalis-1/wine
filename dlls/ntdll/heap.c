...
{
    struct list *head = &heap->free_lists[0].entry;
    struct list *ptr = head;
    struct entry *entry;
    struct block *block;
    SIZE_T scanned = 0;

    while ((ptr = list_next(head, ptr)))
    {
        entry = LIST_ENTRY(ptr, struct entry, entry);
        block = &entry->block;

        /* fast reject first (most likely to fail) */
    ...
    SIZE_T total_size = sizeof(SUBHEAP) + block_size + sizeof(struct entry);
    if (total_size < block_size)
        return NULL;

    SUBHEAP *subheap = create_subheap(
        heap,
        flags,
